...
from monitoring.phase2.model_monitor import PROMPT_VERSIONS, record_model_call
...
def _deepseek_executive_summary(insights: str, *, run_id: str | None = None) -> str:
    ...
    try:
        ...
        record_model_call(
            ...
            prompt_version=PROMPT_VERSIONS["daily_summary"],
            temperature=0.3,
...
def build_report(records: list[dict], date_str: str, *, run_id: str | None = None) -> dict:
    total        = len(records)
    ...
    ai_summary = _deepseek_executive_summary(
        "\n".join(insight_snippets), run_id=run_id
    )
...
def send_daily_report(date_str: str = None, *, run_id: str | None = None):
    if date_str is None:
    ...
    payload = build_report(records, date_str, run_id=run_id)
